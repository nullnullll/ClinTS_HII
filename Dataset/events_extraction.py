...
for id in range(len(list_adm_id)):
    ...
    "FROM inputevents_cv "+
    "where hadm_id = "+str(list_adm_id[id][0])+" AND itemid in(30047,30120,30044,30119,30309,30127, 30128,30051,30043,30307,30042,30306,30125, 42273, 42802)) "+
    ...
    "from inputevents_cv "+
    "where hadm_id ="+ str(list_adm_id[id][0])+" AND itemid in (30008 ,30009 ,42832 ,40548 ,45403 ,44203,30181,46564 ,43237,43353,30012  ,46313 ,30011,30016,42975,42944,46336,46729,40033,45410,42731 ) "+
    ...
    cur.execute(
        "with t1 as (select  starttime as charttime, round(case when amountuom = 'L' then amount * 1000.0 " +
        "when amountuom = 'ml' then amount else null end) as amount from inputevents_mv " +
        "where hadm_id ="+ str(list_adm_id[id][0])+" AND itemid in(225158, 225828,225944, 225797, 225159, 225823, 225825, 225827, 225941,226089) " +
        "and statusdescription != 'Rewritten' " +
